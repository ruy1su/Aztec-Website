...
function ResourceController() {
    ...
    this.ir = function(req, res) { self._ir(self, req, res); };
}

ResourceController.prototype._ir = function (self,req,res){
    p = JSON.parse(decodeURIComponent(req._parsedUrl.query)).params;
//    p[2] = Math.min(p[2], 10);

    request.post({
      url: "http://localhost:8084/aztec-search-ws/search",
      data: {
	jsonrpc: "2.0",
	...
	params: p
        ...
        //console.log(err);
	var entries = JSON.parse(body).result.entries;
	ids = [];
...
ResourceController.prototype._search = function (self,req,res){
    ...
    search.type = BD2K.has(json.type) && json.type == "ir" ? "ir" : "solr";
